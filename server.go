// Mason Willy - CSCE3550
// Using template from Jacob Hochstetler
...
func AuthHandler(w http.ResponseWriter, r *http.Request) {
	...
	acceptType := r.Header.Get("Accept-Type")
	if acceptType == "" {
		acceptType = "*/*"
	}
	t, _, err := mime.ParseMediaType(acceptType)
	if err != nil {
		return
	}
	switch t {
	case "application/json":
		...
		_, _ = w.Write([]byte(`{"jwt":"` + signedToken + `"}`))
	default:
